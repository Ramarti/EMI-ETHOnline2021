...
contract ArbitrableTokenList is PermissionInterface, Arbitrable {
    ...
    function requestStatusChange(
        ...
        address _addr,
        string memory _symbolMultihash
    )
        external
        payable
    {
     ...
     */
    function challengeRequest(bytes32 _tokenID, string memory _evidence) external payable {
     ...
     */
    function fundAppeal(bytes32 memory _tokenID, Party _side) external payable {
     ...
     */
    function withdrawFeesAndRewards(address _beneficiary, bytes32 memory _tokenID, uint _request, uint _round) public {
        Token storage token = tokens[_tokenID];
     ...
     */
    function batchRoundWithdraw(address _beneficiary, bytes32 memory _tokenID, uint _request, uint _cursor, uint _count) public {
        Token storage token = tokens[_tokenID];
        Request storage request = token.requests[_request];
     ...
     */
    function executeRequest(bytes32 memory _tokenID) external {
        Token storage token = tokens[_tokenID];
        Request storage request = token.requests[token.requests.length - 1];
        require(
     ...
     */
    function submitEvidence(bytes32 _tokenID, string memory _evidence) external {
        Token storage token = tokens[_tokenID];
        Request storage request = token.requests[token.requests.length - 1];
     ...
     */
    function changeMetaEvidence(string memory _registrationMetaEvidence, string memory _clearingMetaEvidence) external onlyGovernor {
        metaEvidenceUpdates++;
     ...
     */
    function isPermitted(bytes32 memory _tokenID) external view returns (bool allowed) {
        Token storage token = tokens[_tokenID];
     ...
     */
    function amountWithdrawable(bytes32 memory _tokenID, address _beneficiary, uint _request) external view returns (uint total){
     ...
     */
    function queryTokens(bytes32 calldata _cursor, uint _count, bool[8] calldata _filter, bool _oldestFirst, address _tokenAddr)
        external
        view
        returns (bytes32[] calldata values, bool hasMore)
    {
        uint cursorIndex;
